...
class ThreeDEntryPage extends StatefulWidget {
  final DrawRun draw;
  const ThreeDEntryPage({super.key, required this.draw});

  @override
  State<ThreeDEntryPage> createState() => _ThreeDEntryPageState();
}

class _ThreeDEntryPageState extends State<ThreeDEntryPage> {
  ...
  void initState() {
    ...
    config = EntryConfig.threeD(widget.draw);
  ...
  Future<void> confirmTicket() async {
    ...
    try {
      await TicketService().purchase3DTicket(
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("🎉 3D Ticket placed successfully"),
